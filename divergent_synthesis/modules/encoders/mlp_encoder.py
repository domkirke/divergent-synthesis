...
import math, torch, torch.distributions as dist, torch.nn as nn, numpy as np
from divergent_synthesis.utils import checklist, checksize, checkdist, reshape_batch, flatten_batch
from divergent_synthesis.modules import layers as layers, mlp_dist_hash , Reshape
...
class MLPEncoder(nn.Module):
    ...
    def __init__(self, config: OmegaConf, **kwargs):
        ...
        self.dist_module = nn.Sequential(
            Reshape(*self.target_shape, incoming_dim=1),
            mlp_dist_hash[self.target_dist](out_nnlin=self.out_nnlin)
        )
    ...
    def __call__(self, x: torch.Tensor, return_hidden=False, trace=None, **kwargs) -> Union[torch.Tensor, dist.Distribution]:
        ...
        x = x.view(-1, math.prod(self.input_shape))
